...
| :-- | :-- | :-- | :-- |
| **`/add-dir <path>`** | Utilities | â€” | Add a directory path to the active workspace. |
| **[`/agents`](/docs/cli/commands/agents)** | Tools & Tasks | â€” | Open the [Agent Manager Panel](/docs/cli/commands/agents) to switch custom agents and monitor background subagents. |
| **[`/boost`](/docs/boost)** `<task>` | Reasoning | â€” | Run on-demand multi-agent deep reasoning loops. |
| **`/artifact`** | Tools & Tasks | â€” | Open the Artifact Review Panel. |
| **`/btw <query>`** | Utilities | â€” | Ask a side question in the background without interrupting the main conversation. |
...
| **`/context`** | Utilities | â€” | Open the context usage visualization panel. |
| **`/copy`** | Utilities | â€” | Copy the last agent response to the system clipboard. |
| **[`/credits`](/docs/cli/commands/credits)** | Account | â€” | View remaining G1 credits and purchase links. |
| **[`/diff`](/docs/cli/commands/diff)** | Utilities | â€” | Open the [Interactive Diff Viewer](/docs/cli/commands/diff) to view changes, turns, and commits. |
| **`/exit`** | Core | `/quit` | Close the TUI session and restore your host shell. |
| **`/fast`** | Configurations | â€” | Enable fast mode (bypass reasoning plans) for quick actions. |
| **`/feedback`** | Utilities | â€” | Open the feedback submission panel. |
| **`/fork`** | Conversations | `/branch` | Clone the current conversation thread into a new parallel session. |
| **`/help`** | Utilities | â€” | Open the help panel showing commands and shortcuts. |
| **`/hooks`** | Tools & Tasks | â€” | Browse active pre-flight/post-format script hooks. |
| **`/keybindings`** | Configurations | â€” | Open the interactive Keyboard Shortcut Editor. |
| **`/logout`** | Account | â€” | Disconnect your profile and purge authentication tokens from the secure keyring. |
| **`/mcp`** | Tools & Tasks | â€” | Open the Model Context Protocol (MCP) server manager. |
| **`/model`** | Configurations | â€” | Choose your preferred reasoning model (persists across sessions). |
| **`/open <path>`** | Utilities | â€” | Force the path to open inside your default system editor. |
| **[`/permissions`](/docs/cli/commands/permissions)** | Configurations | â€” | Open the interactive tool permissions manager panel. |
| **`/planning`** | Configurations | â€” | Enable multi-turn plan generation mode for complex engineering tasks. |
| **`/rename <name>`** | Conversations | â€” | Rename the current session thread. |
| **[`/remote-control`](/docs/remote-control?tab=cli#interactive-mode) \[on/off\]** | Utilities | â€” | Toggle Remote Control for the active terminal session. |
...
| **`/skills`** | Tools & Tasks | â€” | Browse loaded local and global Agent Skills. |
| **[`/statusline`](/docs/cli/commands/statusline)** | Configurations | â€” | Open the Status Bar customization overlay. |
| **`/tasks`** | Tools & Tasks | â€” | Open the Task Manager Panel to monitor background shell execution logs. |
...
| **[`/title`](/docs/cli/commands/title) \[on/off\]** | Configurations | â€” | Toggle or set terminal window title updates. |
...
| **`Ctrl+D`** | `â€”` | Forward delete (only when the prompt box is non-empty). |
...
| Key | TUI Command | Action Behavior |
| :-- | :-- | :-- |
| **`â†‘`** / **`â†“`** | `navigation.up` / `navigation.down` | Scrolls highlighted selections up or down by one item. |
| **`PgUp`** / **`Shift+â†‘`** | `navigation.page_up` | Scrolls the active text viewport up by one page block. |
| **`PgDn`** / **`Shift+â†“`** | `navigation.page_down` | Scrolls the active text viewport down by one page block. |
| **`â†`** / **`â†’`** | `navigation.left` / `navigation.right` | Swaps pages inside multipage structures (like the Session Picker). |
...
| **`A`** | `â€”` | (Inside Review Panel) Approves all generated artifacts in one action (built-in shortcut). |
...
| **`allowNonWorkspaceAccess`** | boolean | `false` | Permits the agentâ€™s file read and write tools to navigate outside recognized Git/workspace roots. |
