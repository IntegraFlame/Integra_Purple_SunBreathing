...
Google Antigravity is available with [terms](https://antigravity.google/terms) to individual accounts derived from Googleâ€™s terms of service, and available to teams under Google Cloud terms through the Gemini Enterprise Agent Platform. To learn more, see [Enterprise Get Started](/docs/enterprise).
...
Usage of credits once the baseline quota is exhausted for any particular model is controlled by the â€œAI Credit Overagesâ€ user setting, which can be set to the following:
