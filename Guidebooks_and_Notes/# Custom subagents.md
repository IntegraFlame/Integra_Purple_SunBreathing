...
* **Context Isolation**: The subagent runs using the specified model tier but does not inherit the parentâ€™s existing conversation history (context window), starting with a clean slate.
...
The content following the YAML `---` delimiter defines the subagentâ€™s system prompt. You can organize instructions using standard Markdown H1 headings (`# System Prompt`, `# Review Guidelines`).
...
* **Shared Transcripts**: Agents can read each otherâ€™s conversation transcripts to audit multi-step workflows.
...
* **Inherited Scopes**: Subagents automatically inherit the parentâ€™s allowed terminal command prefixes, file read/write directory scopes, and sandbox settings.
* **Workspace Access**: Parent agents retain full access to their subagentsâ€™ workspaces, including isolated Git worktrees.
...
* **Role**: The specialized role of the agent (such as â€œCodebase Researcherâ€ or â€œDatabase Debuggerâ€).
...
1. Open the `/agents` panel and highlight the target agent using â†‘/â†“.
2. Press Enter to open the **Subagent Detail View**.
3. Inspect the subagentâ€™s entire reasoning log, including its private internal thoughts, tool calls, and execution outputs.
...
* Select a task using â†‘/â†“ and press Enter to view stdout logs.
...
### Detailed â€œTeleportâ€ navigation (`Alt+J`)
...
* Press Alt + J inside the main prompt panel to instantly â€œteleportâ€ from your current conversation directly into the Detail View of the next subagent awaiting your approval.
* Confirm or reject the action, and press Esc to teleport back to your primary thread.

### â€œFast-Pathâ€ confirmations (`Ctrl+K`)
